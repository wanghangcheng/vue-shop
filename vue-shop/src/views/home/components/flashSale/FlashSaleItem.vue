<template>
    <div class="wrapper" data-index="3">
        <div class="itemWrapper">
            ...
            <div>
                <div class="name" v-html="cutout(product.name)"></div>
                <div class="originPrice">{{product.origin_price | moneyFormat}}</div>
                <div class="priceWrapper" @click="addToCart(userInfo,product)">
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            cutout(cellValue) {
                //使用正则/[\s]+/g匹配空格,其中+表示至少一个，g表示全部匹配替换
                return cellValue.replace(/[\s]+/g, '</br>')
            },
            addToCart(userInfo,goods) {
                if (this.userInfo.token) {
                    ...
                    this.dealAddGoods(userInfo,goods);
                    //提示用户
                    Toast({
                        type:'success',
                        message:'添加成功',
            ...
            //向服务端添加商品到购物车进行持久化
            async dealAddGoods(userInfo,goods){
                let result = await addCart(userInfo.token,goods.id,goods.name,goods.price, goods.small_image);
                console.log(result)
            }
        },
    }
